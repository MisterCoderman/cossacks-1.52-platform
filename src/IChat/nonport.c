...
#include <emscripten.h>   /* emscripten_sleep for the msleep() yield below */
#endif
...
{ 
	...
	return 	(DEC(lasttimecalled.day) * 86400000) +
...
{
#ifdef __EMSCRIPTEN__
	// Must be a REAL browser-event-loop yield (Asyncify), not usleep: usleep busy-waits on the
	// wasm main thread, so a CI_DO_BLOCKING loop (chatMain.c) waiting for a server reply would
	// spin forever - the WebSocket onmessage that carries that reply can never run. GSC's own UI
	// code only uses non-blocking SDK calls, but this keeps the SDK's blocking mode safe too.
	emscripten_sleep(msec ? (int)msec : 1);
	return;
#endif
#ifdef _WIN32
	Sleep(msec);
...
#ifndef PATH_MAX
#define PATH_MAX 260   /* MAX_PATH is Windows-only; this branch only runs on UNDER_UNIX anyway */
#endif
