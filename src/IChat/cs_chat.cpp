...
bool ChatSystem::ConnectToChat( char* Nick, char* Info, char* Mail, char* Chat )
{
	...
	printf( "[GSC] chatConnect('%s':%d nick='%s')...\n", serverAddress, port, chatNick );
	...
	printf( "[GSC] chatConnect -> %p\n", (void*) chat );
	int t0 = current_time();
	int dbg_last = t0;
	do
	{
		if (current_time() - dbg_last > 2000)
		{
			dbg_last = current_time();
			printf( "[GSC] connect-wait t=%dms Connected=%d Error=%d\n", dbg_last - t0, (int) Connected, (int) Error );
		}
		ProcessMessages();
		Process();
#ifdef __EMSCRIPTEN__
		// This is a tight CPU poll, not an OS-level blocking wait: on native Windows the kernel
		// receives TCP data on its own thread regardless of what this loop does, but the browser's
		// WebSocket only delivers bytes to onmessage when the JS event loop gets a turn. Without
		// yielding here, the server's NICK/USER reply (001 Welcome) can never be dispatched, so
		// Connected/Error never flip and this spins for the full 40s freezing the tab solid.
		emscripten_sleep( 0 );
#endif
	} while (!( Connected || Error || current_time() - t0 > 40000 ));
	printf( "[GSC] connect-wait done t=%dms Connected=%d Error=%d\n", current_time() - t0, (int) Connected, (int) Error );
	...
	else
	{
		...
		do
		{
			...
			{
				dbg_last = current_time();
				printf( "[GSC] join-wait t=%dms ch0=%d ch1=%d Error=%d\n", dbg_last - t0, (int) enterChannelSuccess[0], (int) enterChannelSuccess[1], (int) Error );
			}
			ProcessMessages();
			Process();
#ifdef __EMSCRIPTEN__
			emscripten_sleep( 0 );   // same reason as the connect-wait loop above
#endif
	...
	printf( "[GSC] ConnectToChat done Connected=%d Error=%d\n", (int) Connected, (int) Error );
	return Connected;
};
