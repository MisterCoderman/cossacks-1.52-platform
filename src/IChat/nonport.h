...
// WASM/browser build: redirect the BSD socket calls above to the WebSocket-tunnel shim (see
// cos_chat_net.h's own header comment). Included HERE, after the real system socket headers, so
// the macros below never mangle those headers' own declarations (e.g. sys/select.h's prototype for
// select() itself) - only actual call sites in the SDK's own .c files, included afterward.
#ifdef __EMSCRIPTEN__
#include "cos_chat_net.h"
#endif
