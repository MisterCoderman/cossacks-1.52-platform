...
static void SetGSCTransportMode( const char* mode )
{
#ifdef __EMSCRIPTEN__
	cosn_set_relay_mode( mode && !strcmp( mode, "relay" ) ? 1 : 0 );
#endif
}

static void FillWasmChatIdentity( char*& Nick, char*& Prof )
{
#ifdef __EMSCRIPTEN__
	// Nick/Prof come from the GSC server via ok_enter.cml's LW_gvar (%NICK/%PROF); this only
	// guards the pre-login default so nothing dereferences an empty identity.
	if (!Nick || !Nick[0] || !strcmp( Nick, "UNKNOWN" ))
	{
		Nick = (char*) "Player";
	}
	if (!Prof || !Prof[0])
...
struct RoomInfo
{
	...
	//Rendezvous (hole punch) port as its OWN field: for a JOINER `port` above must stay the game
	//port from %CG_PORT, so the hole-punch port needs a separate slot (host writes both).
...
{
	printf( "[GSC] ChatWindow enter Active=%d\n", Active );
	GRIF = RIF;
	...
	IniFile INI;
	bool iniok = INI.Load( "Internet\\gscchat.ini" );
	printf( "[GSC] gscchat.ini loaded=%d PALETTE_INDEX=%d\n", iniok, INI.GetInt( "PALETTE_INDEX" ) );
	...
	printf( "[GSC] palette ok '%s'\n", INI.GetStr( "PALETTE_FILE" ) );
	...
	{
		printf( "[GSC] pics ok, CHATSERV='%s'; RunHTTPC...\n", CHATSERV );
		RunHTTPC();
		...
		FillWasmChatIdentity( Nick, Prof );
		if (!Nick)
			Nick = "UNKNOWN";
		if (!Prof)
			Prof = "";
		...
		CSYS.Disconnect();
		printf( "[GSC] ConnectToChat nick='%s' prof='%s' serv='%s'...\n", Nick, PRF, CHATSERV );
		CSYS.ConnectToChat( Nick, PRF, Mail, CHATSERV );
		printf( "[GSC] ConnectToChat -> Connected=%d\n", (int) CSYS.Connected );
		if (!CSYS.Connected)
		{
			Back.Draw( menu_x_off, menu_y_off );
			DarkScreen();
			...
			{
				ProcessMessages();
				// FlipPages() is this engine's per-frame yield point (emscripten_sleep under the
				// hood) - without calling it again here, the browser's keyboard events backing
				// KeyPressed can never be delivered and this spins forever, freezing the tab solid.
				FlipPages();
			} while (!KeyPressed);
			return false;
		}
	...
	if (Active)
	{
		printf( "[GSC] RunExplorer(home.cmd)\n" );
	...
	do
	{
		...
		if (!CSYS.Connected)
		{
			...
			if (!CSYS.Connected)
			{
				...
				{
					ProcessMessages();
					FlipPages();   // see the same wait loop above for why this yield is required
				} while (!KeyPressed);
				return false;
			}
		}
						...
						if (MaxPL)
						{
							...
							if (hole_port_str)
							{
								...
								{//Conversion successful, string countained one number only
									...
									RIF->udp_port = (unsigned short) port;
								}
							}
							//Server ip
							...
							SetGSCTransportMode( ExplorerGetVar( 0, "%CG_TRANSPORT" ) );
							return 1;
						}
					}
				}
						...
						if (GAMENAME)
						{
							...
							if (MaxPL)
							{
								...
								if (GAMEIP)
								{
									...
									//Hole-punch rendezvous data for the JOINER: without these the
									//joiner never registers on the rendezvous (no CSHP), the server
									//never learns its punched address, and canExchangeUdp() fails
									//for BOTH sides - every "udp" room silently degraded to relay.
									const char *player_id_str = ExplorerGetVar( 0, "%PROF" );
									if (player_id_str)
									{
										char *end;
									...
									{
										char *end;
										const unsigned long hole_port = std::strtoul( hole_port_str, &end, 10 );
										if (hole_port && *end == '\0')
										{
											RIF->udp_port = (unsigned short) hole_port;
										}
									}
									const char *hole_interval_str = ExplorerGetVar( 0, "%CG_HOLEINT" );
									if (hole_interval_str)
