...
void WaitWithError( char* ID, int GPID )
{
	...
	{
		ProcessMessages();
		// FlipPages() is this engine's per-frame yield point (emscripten_sleep under the hood) -
		// without calling it again here, the browser's keyboard events backing KeyPressed can never
		// be delivered and this spins forever, freezing the tab solid instead of waiting for a key.
		FlipPages();
	} while (!KeyPressed);
}
...
void WaitWithError4( char* ID, int GPID )
{
	...
	{
		ProcessMessages();
		FlipPages();   // see WaitWithError above for why this yield is required
	} while (!KeyPressed);
}
...
{
	if (!L)return;
	Chunks = (OneIChunk**) realloc( Chunks, (N + 1) * sizeof(OneIChunk*) );
