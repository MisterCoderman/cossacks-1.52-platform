...
static void ResolveUserMissionLaunchPaths(const char* inPath, char* mapOut, int mapOutSize, char* dllOut, int dllOutSize, char* briefOut, int briefOutSize);
...
__declspec(dllexport) bool MMItemChoose(SimpleDialog* SD)
{
	...
	fprintf(stderr, "[CLICK-DBG] MMItemChoose UserParam=%d\n", SD->UserParam);
	Lpressed = false;
	return true;
}
...
//Changes resolution
#ifdef __EMSCRIPTEN__
extern "C" int cos_browser_fullscreen(void);   // Ddini.cpp: browser-fullscreen state
extern "C" void cos_viewport_changed(void);    // Ddini.cpp: refit canvas backing + resolution
#endif
...
#ifdef __EMSCRIPTEN__
	// Web: resolution is AUTO in a window (the game always fills the tab, so mouse maps 1:1 and a
	// mode larger than the tab can't break input) and is only user-selectable in BROWSER-FULLSCREEN.
	// Remember the pick for fullscreen; apply it only when actually fullscreen. cos_viewport_changed()
	// then fits the canvas backing to the tab and letterboxes the frame.
	exRealLx = SizeX; exRealLy = SizeY;
	if (cos_browser_fullscreen()) { RealLx = SizeX; RealLy = SizeY; }
	InitScreen();
	GSSetup800();
	DrawAllScreen();
	cos_viewport_changed();
	ResetMouseCapture();
	return true;
#else
	RealLx = SizeX;
...
extern "C" int cosn_has_native_udp(void);
extern "C" void cosn_set_relay_mode(int relayFlag);
// Etap 2 rendezvous: announce a GSC internet game to the master by the HOST's relay peer id so the
// server groups host+joiners and can fan out real punched addresses; carries the CSHP identity
// (GlobalRIF.player_id + login ACCESS key) so the UDP packets correlate to this relay session.
extern "C" void cosn_gsc_room(int hostId, int punchId, const char* key, int isHost);
extern "C" int cosn_my_relay_id(void);
extern "C" void cosn_gsc_leave(void);
...
RetryConn:
	...
	{
		cosn_set_relay_mode(0);
		// Leaving the GSC/internet path: drop internet-room state so a LAN/direct-IP game that
		// follows uses pure native UDP (real addresses, broadcast) instead of the relay/punch routing.
		cosn_gsc_leave();
	}
	if (selected_network_protocol == 3)
	{
	...
	REC3:
		// NO local EnterName() here: the GSC server itself serves the registration/nick form
		// (enter.cml -> "Your nick"). Asking for a name locally first would double-prompt.
		//Show login menu
		printf("[GSC] processMultiplayer: protocol=3, login...\n");
		if (!ProcessNewInternetLogin())
		{
			printf("[GSC] login cancelled/failed\n");
			return false;
		}

	REINCONN:
		printf("[GSC] -> ProcessInternetConnection(1)\n");
		int r = ProcessInternetConnection(1);
		printf("[GSC] ProcessInternetConnection -> %d\n", r);
		if (!r)
		{
			return 0;
		}
	...
	case 11://Inet Host(Deathmatch)
		...
		if (CreateSession(GlobalRIF.Name, GlobalRIF.Nick, 0, DoNewInet, GlobalRIF.MaxPlayers))
		{
			...
			// Only a process with a real, unrestricted UDP socket (Electron) can attempt NAT hole
			// punching - a browser peer has no raw-socket API to receive an unsolicited inbound
			// punch reply on, so its keepalive packets could only ever be dropped en route.
			if (cosn_has_native_udp())
			{
				// Announce the game by OUR relay id (this is the host); it equals the id the joiner
				// derives from %CG_IP=10.<id>, so both land in the same rendezvous group. Send it
				// BEFORE the first CSHP so the server has re-keyed our punch identity when CSHP lands.
				cosn_gsc_room(cosn_my_relay_id(), (int)GlobalRIF.player_id, ACCESS, 1);
				udp_hole_puncher.Init(GlobalRIF.udp_server, GlobalRIF.udp_port, GlobalRIF.udp_interval,
					GlobalRIF.player_id, ACCESS);
				// fire the first CSHP right away: the rendezvous must learn our punched address
				// before any joiner asks for it (the server briefly delays 'joined' waiting for it)
	...
	case 10://Inet Join(Deathmatch)
		...
		// The JOINER must hole-punch too: its CSHP keepalives (a) register its punched address on
		// the rendezvous so the server can fan it out to the host, and (b) open this NAT towards
		// the server. Without this the host never learns where to punch back and every "udp" room
		// silently fell back to relay. Mirrors the host path in case 11.
		if (cosn_has_native_udp())
		{
			// The host advertised %CG_IP=10.a.b.c; derive its relay id (idFromIp = a<<16|b<<8|c) so we
			// announce the SAME rendezvous group the host did. Send before the first CSHP.
			int hia = 0, hib = 0, hic = 0;
			sscanf(GlobalRIF.RoomIP, "10.%d.%d.%d", &hia, &hib, &hic);
			int hostRelayId = ((hia & 255) << 16) | ((hib & 255) << 8) | (hic & 255);
			cosn_gsc_room(hostRelayId, (int)GlobalRIF.player_id, ACCESS, 0);
			udp_hole_puncher.Init(GlobalRIF.udp_server, GlobalRIF.udp_port, GlobalRIF.udp_interval,
				GlobalRIF.player_id, ACCESS);
			udp_hole_puncher.KeepAlive();
		}
...
bool MMChooseName(SimpleDialog* SD)
{
	...
	fprintf(stderr, "[CLICK-DBG] MMChooseName UserParam=%d\n", SD->UserParam);
	Lpressed = false;
	return false;
}
...
ffe2:
	...
	do
	{
		...
		if (Host)
		{
			// StartLanServerAdvertise() only opens the discovery socket; nothing ever drains it
			// without this per-frame poll, so a hosted LAN game never answers joiners' broadcast
			// queries even after the WS relay is reachable.
			PollLanServerAdvertise();
		}
...
bool ProcessMenuOptions()
{
	...
	REC->UserParam = 77;
	bool RECSTATE = RecordMode;//Recording checkbox reflects the real state (OFF by default)
	//Recodring file path
...
#ifdef __EMSCRIPTEN__
		{
			// Mark the monitor's TRUE native mode; it is also the fullscreen default.
			extern int cos_native_w, cos_native_h;
			if (ModeLX[i] == cos_native_w && ModeLY[i] == cos_native_h)
				sprintf(cc, "%dx%d NATIVE", ModeLX[i], ModeLY[i]);
			else
	...
	do
	{
		if (RECSTATE) // checkbox reflects the real recording state (OFF by default)
		{
			EDITPAN->Enabled = 1;
			IBREC->Enabled = 1;
			REC->ActiveFrame = 22;
	...
	if (ItemChoose == mcmOk)
	{
		...
		RecordMode = RECSTATE;
#ifdef __EMSCRIPTEN__
		// Persist NOW: on Windows these globals were written to mode.dat only at process exit,
		// which never happens in a browser tab - so Apply itself saves them.
		{ extern void SaveModeDatNow(void); SaveModeDatNow(); }
#endif
	}
	else
	{
...
bool ProcessGameOptions()
{
	...
	if (mcmOk == ItemChoose)
	{
		...
		PlayMode = MUS->CurLine;
#ifdef __EMSCRIPTEN__
		// Persist NOW (in-game Options Apply) - the browser never runs the shutdown writer.
		{ extern void SaveModeDatNow(void); SaveModeDatNow(); }
#endif
	}
	else
...
int processMainMenu()
{
	...
	while (true)
	{
		...
		if (ItemChoose == 111)
		{
			...
			char userMissionMap[260];
			char userMissionDll[260];
			char userMissionBriefing[260];
			ResolveUserMissionLaunchPaths(USERMISSPATH, userMissionMap, sizeof(userMissionMap),
				userMissionDll, sizeof(userMissionDll), userMissionBriefing, sizeof(userMissionBriefing));
			memset(PINFO, 0, sizeof(PINFO));
			strcpy(PINFO[0].name, PlName);
			EditMapMode = 0;
			ItemChoose = mcmSingle;
			strcpy(CurrentMap, userMissionMap);
			...
			InitGame();
			if (!SCENINF.ScenaryHandler)
			{
				SCENINF.Load(userMissionDll, userMissionBriefing);
			}
			ItemChoose = mcmSingle;
			RUNUSERMISSION = 0;
		}
...
StartPlay://IMPORTANT: Main game loop
	...
	// Web: InGame is still true here (processMainMenu clears it later), so tell the web display
	// path explicitly that this request IS the fixed menu mode — otherwise it overwrote 1024x768
	// with the tab size and left stale mouse/draw clamps (dead bottom/right menu zone).
	{
		extern int g_cos_menu_fixed_req;
		g_cos_menu_fixed_req = 1;
		if (window_mode)
...
void EditGame()
{
	...
	//Reset resolution for main menu (web: mark as the fixed MENU mode — see PlayGame tail)
	{
		extern int g_cos_menu_fixed_req;
		g_cos_menu_fixed_req = 1;
		if (window_mode)
...
// 'pilot' cheat (single-player): hand the human player's nation to the AI, or take it back.
// Toggling ON starts the same StartAI path the bots use, so it plays economy + military on the
// player's OWN nation; toggling OFF stops the AI tick for that nation so FindNeedProject() no
// longer calls ProcessAIinDLL and the human regains control.
//
// aiName must be a valid GetTextByID() key naming this nation's culture ("<NatID-name>.<n>") —
// StartAIEx resolves it to the actual AI DLL. The original version of this cheat found that key by
// searching for ANOTHER nation that had already gone through StartAIEx and copying its AIID — but
// that only ever gets populated for random-map/skirmish bots (Interface.cpp's else-branch around
// COMPSTART[i]); scenario/historical-battle missions set up their AI from the mission DLL/script
// instead and never touch CITY[i].AIID, so on those maps the search always came up empty and the
// cheat silently did nothing. Deriving the key directly from the player's OWN nation (same
// Group[]-scan-for-a-peasant -> NatID -> NatsIDS[NatID] lookup the working call site already uses)
// needs no other bot to exist first and works on every map type.
extern byte AssignTBL[8];
extern byte PlayGameMode;
static bool g_pilot_active = false;
void TogglePilot()
{
	if (PlayGameMode == 1 || PlayGameMode == 2) return;   // watching/running a recorded stream -> refuse (SP-play only)
	byte Nat = MyNation;
	...
	if (!g_pilot_active)
	{
		...
		for (int j = 0; j < MAXOBJECT; j++)
		{
			...
			if (OB && OB->NNUM == Nat && (!OB->Sdoxlo) && OB->newMons->Peasant)
			{
				NatID = OB->Ref.General->NatID;
				break;
			}
		}
		if (NatID == -1)
		{
			fprintf(stderr, "[PILOT] no peasant found for nation %d — can't determine AI culture\n", Nat);
			return;
		}
		char aiName[48];
		snprintf(aiName, sizeof aiName, "%s.%d", NatsIDS[NatID], 0);
		int land = 0, money = CITY[Nat].StartRes, resmap = CITY[Nat].ResOnMap, diff = 2;
		StartAI(Nat, aiName, land, money, resmap, diff);
		g_pilot_active = true;
		fprintf(stderr, "[PILOT] ON: AI drives player nation %d ('%s')\n", Nat, aiName);
	}
	else
	{
		byte AN = AssignTBL[Nat];
		NATIONS[AN].AI_Enabled = 0;
		NATIONS[AN].ProcessAIinDLL = 0;         // AI tick no longer fires -> player controls again
		g_pilot_active = false;
		fprintf(stderr, "[PILOT] OFF: control returned to player nation %d\n", Nat);
	}
}
...
#ifdef __EMSCRIPTEN__
extern "C" int cos_fetch_file(const char* canonPath);   // lazy-fetch backend (web/cos_data.js)
#endif

static void GetUserMissionLang(char lang[3])
{
	strcpy(lang, "en");
#ifdef __EMSCRIPTEN__
	int isru = EM_ASM_INT({ return (typeof COS_DATA_STATE !== 'undefined' && COS_DATA_STATE.base === 'data/ru') ? 1 : 0; });
	strcpy(lang, isru ? "ru" : "en");
#else
	LCID lcid = GetSystemDefaultUILanguage();
	if (GetLocaleInfoA(lcid, LOCALE_SISO639LANGNAME, lang, 3) == 0)
		strcpy(lang, "en");
#endif
}

static bool UserMissionFileExists(const char* winPath)
{
#ifdef __EMSCRIPTEN__
	char canon[300];
	int i = 0;
	for (const char* q = winPath; *q && i < 299; q++)
	{
		char c = *q;
		if (c == '\\') c = '/';
		canon[i++] = (char)toupper((unsigned char)c);
	}
	canon[i] = 0;
	return cos_fetch_file(canon) == 0;
#else
	WIN32_FIND_DATAA fd;
	HANDLE h = FindFirstFileA(winPath, &fd);
...
static void GetUserMissionScriptBase(const char* dllName, char* out, int outSize)
{
	strncpy(out, dllName, outSize - 1);
	out[outSize - 1] = 0;
	_strlwr(out);
	for (char* p = out; *p; p++) if (*p == '\\') *p = '/';

	char* ext = strstr(out, ".dll");
	if (!ext) ext = strstr(out, ".cms");
	if (!ext) ext = strstr(out, ".m3d");
	if (ext) *ext = 0;
}

static void GetUserMissionBriefingPath(const char* dllName, char* out, int outSize)
{
	char lang[3];
	char base[260];
	char local[260];
	...
	GetUserMissionLang(lang);
	GetUserMissionScriptBase(dllName, base, sizeof(base));
	sprintf(local, "%s_%s.txt", base, lang);
	sprintf(fallback, "%s.txt", base);

	strncpy(out, UserMissionFileExists(local) ? local : fallback, outSize - 1);
	out[outSize - 1] = 0;
}
...
{
	char norm[260];
	strncpy(norm, inPath, sizeof(norm) - 1);
	norm[sizeof(norm) - 1] = 0;
	for (char* p = norm; *p; p++) if (*p == '/') *p = '\\';

	char base[128];
	const char* name = strrchr(norm, '\\');
	name = name ? name + 1 : norm;
	...
	char low[260];
	strncpy(low, norm, sizeof(low) - 1);
	low[sizeof(low) - 1] = 0;
	_strlwr(low);

	if (strstr(low, ".cms") || strstr(low, ".dll"))
	{
		snprintf(mapOut, mapOutSize, "%s.m3d", base);
		strncpy(dllOut, norm, dllOutSize - 1);
		dllOut[dllOutSize - 1] = 0;
	}
	else
	{
		strncpy(mapOut, norm, mapOutSize - 1);
		mapOut[mapOutSize - 1] = 0;
		snprintf(dllOut, dllOutSize, "UserMissions\\%s\\%s.cms", base, base);
	}

	GetUserMissionBriefingPath(dllOut, briefOut, briefOutSize);
}
...
struct OneAddMission
{
	...
	char Preview[128];
	char Bitmap[128];
	char DllName[200];    // the .add file's 3rd line verbatim - passed to SCENINF.Load() to run
	                       // the mission's actual script/DLL; without this the map loads with no
	                       // scenario handler driving it at all (freezes, no win/lose conditions).
	char Briefing[200];   // "<dllbase>[_lang].txt" - #BRIEFING-tagged text SCENINF.Load() parses
	                       // into in-game briefing pages, same role as story missions' Description.
};
...
AddMissionsPack::AddMissionsPack()
{
...
#ifdef __EMSCRIPTEN__
	// Manifest-backed enumeration works before the lazy data files have been fetched into MEMFS.
	TGSCFindInfo* fd = GSFILES.gFindFirst("UserMissions\\*.add");
	if (!fd) return;

	do
	{
		char fname[128];
		strcpy(fname, fd->m_FileName);
#else
	WIN32_FIND_DATAA fd;
	...
	do
	{
		...
		*dot = 0;
		...
		if (UserMissionFileExists(localizedAdd))
		{
		...
		strncpy(Pack[NMiss].DllName, dllname, 199); Pack[NMiss].DllName[199] = 0;
		...
		if (ext)
		{
			...
			*ext = 0;
			...
			if (UserMissionFileExists(dscLocal))
			{
			...
			GetUserMissionBriefingPath(dllname, Pack[NMiss].Briefing, sizeof(Pack[NMiss].Briefing));

			char bmpPath[260];
			sprintf(bmpPath, "%s.bmp", dllname_lc);
			if (UserMissionFileExists(bmpPath))
			{
		...
		for (char* p = Pack[NMiss].Briefing; *p; p++) { if (*p == '\\') *p = '/'; }
#endif
...
bool SelectSingleMission()
{
	...
	*/
	...
	DIFF->AddLine(GetTextByID("@RMID_VERYHARD"));
	DIFF->Hint = GetTextByID("DIFFICULTY");
	// TV/TSB are indexed by (MISSLIST.MSMiss + AMSP.NMiss): one TextViewer per story-single mission,
	// then one per user (.add) mission. The old fixed [128] overflowed the STACK the moment that
	// total passed 128 (e.g. after importing a whole user-mission campaign), corrupting adjacent
	// stack state — hence garbled map previews, a hung menu, and an instant defeat on entering ANY
	// mission. Size to the real total (heap-backed, auto-freed) so any number of missions is safe.
	int MissTotal = MISSLIST.MSMiss + AMSP.NMiss;
	if (MissTotal < 1) MissTotal = 1;
	std::vector<TextViewer*> TVvec(MissTotal, nullptr);
	std::vector<VScrollBar*> TSBvec(MissTotal, nullptr);
	TextViewer** TV = TVvec.data();
	VScrollBar** TSB = TSBvec.data();
	...
	for (int i = 0; i < AMSP.NMiss; i++)
	{
		...
		TV[N0 + i]->SymSize = 21;
		TV[N0 + i]->PageSize = (TV[N0 + i]->y1 - TV[N0 + i]->y) / TV[N0 + i]->SymSize;
		TV[N0 + i]->Visible = 0;
		TSB[N0 + i] = MMenu.addNewGP_VScrollBar(nullptr, 777, 468, 161, 1, 0, SCROL.GPID, 0);
		TSB[N0 + i]->Visible = 0;
		TSB[N0 + i]->ScrDy = 7;
		TV[N0 + i]->AssignScroll(TSB[N0 + i]);
	};
	VideoButton* OkBtn = MMenu.addVideoButton(nullptr, 862, 568, OK1.GPID, OK2.GPID);
	OkBtn->OnUserClick = &MMItemChoose;
	OkBtn->UserParam = mcmOk;
...
HHH1:
	...
	if (ItemChoose == mcmOk)
	{
		...
		else
		{
			...
			// Story missions load their scenario DLL here too (see ProcessSingleMission's
			// identical SCENINF.Load call) - without this the map initializes with no script
			// driving win/lose conditions or events at all, which reads as an immediate freeze
			// once InitGame() finishes (nothing was ever missing visually, nothing was EVER
			// going to happen next).
			SCENINF.Load(AMSP.Pack[LB->CurItem - N0].DllName, AMSP.Pack[LB->CurItem - N0].Briefing);
		};
	}
	else
	{
...
KRT:
	...
	{//Enter key
		...
		{//IMPORTANT: Handling cheats
			...
			if (!strcmp(ChatString, "ai"))
			{
				void TogglePilot();
				TogglePilot();          // SP: toggle AI control of the player's nation
			}
