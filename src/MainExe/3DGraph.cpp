...
#else
	{
		word a = VertBuf[0], b = VertBuf[1]; // MSVC-asm VertBuf[2] = byte offset 2 = word index 1, not C element 2
...
extern byte* tex1;
extern int   MaxTex;
extern char* TexNames[256];
void OverTriangle::Clear() {
	// Освобождаем память для VO->Data в каждом TRIANG
	for (int i = 0; i < VertLx * VertLx; i++) {
		if (TRIANG[i]) {
			for (int j = 0; j < NTRIANG[i]; j++) {
				if (TRIANG[i][j].Data) {
					delete[] TRIANG[i][j].Data;
					TRIANG[i][j].Data = nullptr;
				}
			}
			free(TRIANG[i]);
			TRIANG[i] = nullptr;
	...
	// Освобождаем каждый буфер строки из Buffer
	...
	if (MaxElm) {
		...
		// Освобождаем глобальные графические буферы
		if (tex1) {
			delete[] tex1;
			tex1 = nullptr;
		}
