...
// ZBUF packs this index together with edge flags (0x2000/0x4000/0x8000) in ONE 16-bit word, so the
// index range is bits 0..12 -> MAX 8192. 8192 == the 0x2000 flag boundary and doubles the old 4096,
// which covers 4K screens. Extraction must MASK (SOBJ & (kZCacheMaxElements-1)), never `% N`, because
// modulo only strips flags that are multiples of N (true for 4096, FALSE for 8192+).
const int kZCacheMaxElements = 8192;
...
#define NYLines 4400   /* covers 2160p + DYZBuf margin + tall sprites (was 2000) */
#define DYZBuf 256
#define ZSHF  8        /* 256 sprites per scanline (was 64) for wide screens */
#define ZBFNX (1<<ZSHF)//64

word ZBFCounter[NYLines];   /* was byte: with ZBFNX=256 a byte counter wraps 255->0 and corrupts */
...
void ClearZBuffer()
{
	memset( ZBFCounter, 0, sizeof( ZBFCounter ) );
	NZElm = 0;
	NLOITEMS = 0;
	NHIITEMS = 0;
	NBSel = 0;
			...
			if (nn)
			{
				...
				if (FindXMin)
				{
					for (int j = 0; j < nn; j++)
					{
						...
						if (SOBJ != 0xFFFF)
						{
							ZElement* ZEL = ZCache + ( SOBJ & (kZCacheMaxElements - 1) );
							if (!ZEL->IsDrawn)
							{
