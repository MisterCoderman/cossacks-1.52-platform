...
void HandleMission()
{
	...
	{
		static int scen_first = 1;
		if ( scen_first ) { scen_first = 0; printf( "[SCEN] first ProcessScenary tick\n" ); }
		SCENINF.ScenaryHandler();
	}
...
{
	printf( "[SCEN-ERR] %s\n", Mess );   /* stdout: visible in the browser console (stderr is freopen'd to a MEMFS file) */   // always visible (was gated on a debug ERR.LOG file)
...
void ScenaryInterface::Load(char* Name, char* Text)
{
...
#ifdef __EMSCRIPTEN__
	// Web: localize by the ACTIVE game language (URL ?lang=, exposed by cos_data.js as
	// COS_DATA_STATE.base), NOT the OS UI language — GetSystemDefaultUILanguage is a hardcoded
	// English stub here, which forced Mission_en.txt and lost the Russian mission text.
	...
	else
	{
		...
		// Pure-WASM port: mission DLLs are ORIGINAL PEs run by the cos plugin loader — pass the
		// game-relative name through unchanged. Ildar's ".dll"->".so" rewrite defeated the exact-
		// path module match, so the ambiguous "Mission" stem fallback loaded the FIRST Mission.dll
		// (Algeria/Missal01) for EVERY mission -> wrong script on every map -> 100+ registration
		// errors -> handler silently NULLed. That was the "no mission works" root.
	...
	else
	{
		...
		NErrors = 0;
		{ extern int g_interr_printed; g_interr_printed = 0; }   // fresh console quota per script
		// Diagnostic: what the mission's OnInit registration will see. NAGroups==0 here means the
		// map's named unit-groups are not loaded (ordering/map issue) -> every RegisterUnits fails.
		printf( "[SCEN] Load '%s': NAGroups=%d NAZones=%d before OnInit\n", Name, NAGroups, NAZones );
#ifndef _MSC_VER
		...
		if (fnOnInit) {
			fnOnInit();
			printf( "[SCEN] OnInit() done, NErrors=%d\n", NErrors );
		} else {
		...
		if (NErrors)
		{
			// ORIGINAL Windows behavior: registration ran in DllMain (inside LoadLibrary) and the
			// NErrors=0 reset AFTER LoadLibrary wiped any failures — dangling script references
			// (e.g. Single1 registers GErase/Z1A/Z2A/Z3A that are absent from its .m3d) never
			// disabled a mission. Ildar's port registers via the explicit OnInit call AFTER the
			// reset, which turned those tolerated content quirks fatal (handler=NULL -> "mission
			// does nothing" for ~90% of missions). Match the original: log and keep the script.
			printf( "[SCEN] %d registration error(s) tolerated (original wiped them); script stays ENABLED\n", NErrors );
		}
		printf( "[SCEN] Load result: handler=%s NErrors=%d\n",
		         ScenaryHandler ? "OK (script will tick)" : "NULL (script DISABLED)", NErrors );
	}

	// SAFETY NET (mirrors the replay path, Recorder.cpp ReadStream): a fresh mission MUST start
	// with clean end-of-game state. A rogue script pass during DLL load (or leftovers from the
	// previous mission) could leave Victory/LooseGame latched or a CmdEndGame (cmd 92) sitting in
	// ExBuf — either one makes the new mission end instantly with zero ticks.
	Victory = 0;
	LooseGame = 0;
	PINFO[0].VictCond = 0;
	NATIONS[0].VictState = 0;
	{ extern int EBPos; EBPos = 0; }
}
...
int g_interr_printed = 0;   // console quota, reset per SCENINF.Load
void IntErr( char* Mess )
{
	// The ORIGINAL engine logs these only when a debug ERR.LOG exists — they are tolerated
	// content quirks of the stock missions, not errors (verified: identical sets on the user's
	// original files). Keep a few lines for diagnosability, then go quiet so a mission start
	// doesn't flood the console with red noise.
	if ( g_interr_printed < 12 )
	{
		printf( "[SCEN-note] %s\n", Mess );   /* stdout -> browser console */
		if ( ++g_interr_printed == 12 )
			printf( "[SCEN-note] further script notes suppressed (content quirks the original engine tolerates silently)\n" );
	}
	ResFile FFF = RReset( "ERR.LOG" );
	if ( FFF != INVALID_HANDLE_VALUE )
	{
...
// Used by the x86 game_api marshaler (m_ShowPageParam) to learn a page's format string, so it can
// forward the correct number/type of variadic args from the guest stack. Returns 0 if not found.
extern "C" const char* cos_scen_page_fmt( const char* Name )
{
	for ( int i = 0; i < SCENINF.NPages; i++ )
		if ( !strcmp( Name, SCENINF.PageID[i] ) ) return SCENINF.Page[i];
	return 0;
}
...
void LoadAIFromDLL( byte Nat, char* Name )
{
	...
	if ( !NT->hLibAI )
	{
		fprintf( stderr, "[AI] FAILED: '%s' not in recompiled plugin manifest\n", Name ? Name : "(null)" );
		return;
	}
	NT->ProcessAIinDLL = (VoidProc*) GetProcAddress( NT->hLibAI, "ProcessAI" );
	VoidProc* INITAI = (VoidProc*) GetProcAddress( NT->hLibAI, "InitAI" );
	if ( !NT->ProcessAIinDLL || !INITAI )
	{
		fprintf( stderr, "[AI] FAILED: '%s' missing ProcessAI/InitAI export\n", Name );
		return;
	}
	AiIsRunNow = true;
	INITAI();
