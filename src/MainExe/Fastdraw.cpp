...
void ShowRLCItem(int x, int y, lpRLCTable lprt, int n, byte nt)
{
	...
	// Bound the glyph/sprite index against the actual OfsTable entry count (SCount&0xFFFF,
	// exactly what LoadRLC filled). An out-of-range char routed through an RLC-pointer font
	// would otherwise deref an uninitialized OfsTable slot -> wild pointer -> OOB. GetRLCWidth
	// already guards n<SCount; ShowRLCItem did not.
	{
		int rlc_idx = (n < 4096) ? n : (n - 4096);
		if ((unsigned)rlc_idx >= ((unsigned)(*lprt)->SCount & 0xFFFFu)) return;
	}
