...
// Precomputed trig tables for cross-platform determinism.
// Original code used sin()/cos()/atan2() at runtime, which can produce
// different results on ARM64 vs x86_64 due to FPU implementation differences.
// A ±1 difference in these tables cascades through OneStepDX/DY → RealX/Y →
// unit arrival timing → different rando() call patterns → multiplayer desync.
static const short precomputed_TSin[257] = {
...
static const short precomputed_TCos[257] = {
...
static const short precomputed_TAtg[257] = {
...
void InitExplosions()
{
	...
	memcpy( TSin, precomputed_TSin, sizeof( precomputed_TSin ) );
	memcpy( TCos, precomputed_TCos, sizeof( precomputed_TCos ) );
	memcpy( TAtg, precomputed_TAtg, sizeof( precomputed_TAtg ) );
