...
// ---- on-disk layout under the leaked #pragma pack(1) (MapDiscr.h -> TopoGraf.h -> ActiveZone.h) ----
// These structs are compiled under pack(1) (verified: MapDiscr.h:18 sets pack(1) with no pop, and
// SaveNewMap.cpp includes it before TopoGraf.h/ActiveZone.h), so sizeof(Area)=85, ActiveZone=17,
// ActiveGroup=49 — and the original editor wrote the .m3d with exactly this pack(1) stride. We
// serialise field-by-field (identical byte layout, but explicit — never writes live pointers to
// disk). Do NOT "fix" these to 92/20/52: that assumes default alignment and would desync the map.
...
// ActiveZone under pack(1): int x(4)+int y(4)+int R(4)+char* Name(4)+byte Dir(1) = 17
...
// ActiveGroup under pack(1): word* Units(4)+word* Serials(4)+int N(4)+char* Name(4)+8*int(32)+bool(1) = 49
