...
bool FilesInit()
{
	if (InitDone)
	{
		// Loose-file (fetch) backend: game resources are always available. Do NOT gate on
		// m_ArchList — there is no archive list, and static-init order can reset it to NULL
		// (GSFILES's ctor may run after a global like GConstant already triggered gOpen()).
		return true;
	}
