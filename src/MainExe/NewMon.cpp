...
//*******************************************************//
#include <cstring> // Убедимся, что заголовок подключён для std::memset
#include <cstdint> // Добавляем этот заголовок для определения uint32_t
...
void GetUnitCost(byte NI, word NIndex, int* Cost)
{
	...
	for (int i = 0; i < 8; i++)
		Cost[i] = ADC->NeedRes[i];
	//отключаем для godmod увеличения цены в депо
	if (GodMode) return;
	if (nocost) return;
...
{
	// выключаем увеличение цены юнитов в этом режиме
