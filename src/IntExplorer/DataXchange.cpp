...
void SXP_DevScope::RegisterDevice( char* ID, char* DLL_Path )
{
	...
	if (!strcmp( ID, "LF" ))
	{
		...
		memset( DEVS + NDEVS, 0, sizeof(OneSXP_Device) );
	...
	if (!strcmp( ID, "GW" ))
	{
		...
		memset( DEVS + NDEVS, 0, sizeof(OneSXP_Device) );
		strcpy( DEVS[NDEVS].Name, ID );
		DEVS[NDEVS].SendRequest = &GW_SendRequest;
		DEVS[NDEVS].GetRequestResult = &GW_GetRequestResult;
		DEVS[NDEVS].CloseRequest = &GW_CloseRequest;
		DEVS[NDEVS].Process = &GW_Process;
		DEVS[NDEVS].CloseAll = &GW_CloseAll;
		NDEVS++;
		return;
	}
#endif
...
void exec_LW_tbl( int Np, char** par, int* size )
{
	...
	if (widx >= '0'&&widx <= '7')
	{
		...
		printf( "[TBL] LW_tbl win=%s table='%s' SXT=%p Nne=%s\n", win, par[0], (void*) SXT, par[1] );
		if (SXT)
		{
			int nc = SXT->NCol;
			int Nne = 0;
			Nne = atoi( par[1] );
			printf( "[TBL] nc=%d Nne=%d Np=%d first='%s'\n", nc, Nne, Np, Np > 2 ? par[2] : "-" );
...
void FilterRQ2Send( sicExplorer* SXP, ParsedRQ* RQ, bool AllowNew )
{
	...
	for (int i = 0; i < RQ->NComm; i++)
	{
		...
		if (cid[0] == 'L'&&cid[1] == 'W'&&cid[2] == '_')
		{
			...
			{
				// bound-check like LW_show above: the lobby fires "LW_gvar&%ASTATE&..." via
				// ExplorerOpenRef every 2s - with no window open this dereferenced garbage
				if (SXP->CurWPosition < SXP->NWindows)
				{
					char* win = SXP->Windows[SXP->CurWPosition]->WinID;
					RQ->AddParamToCom( i, win, strlen( win ) );
