...
static inline int ShellExecute( void*, const char*, const char*, const char*, const char*, int )
{
	return 0;
}
#endif
...
void OneSicWindow::Process()
{
	...
	else
	{
		...
		if (!Parsed)
		{
			printf( "[GSC] SXP parse begin win='%s' len=%d\n", WinID, Result ? (int) strlen( Result ) : -1 );
			int pos = ParseTheWholeText();
			printf( "[GSC] SXP parse done pos=%d\n", pos );
			LoadCookies();
			if (Result&&pos != -1)
			{
	...
	while (s[0] != ','&&s[0] != ')'&&s[0] != ']'&&s[0])
	{
		...
		else
		{
			...
			if (L)
			{
				...
				// s += L - 1 must stay INSIDE if(L): with L==0 (unparseable char, e.g. a literal
				// '<' from an unsubstituted <#WinW#>) the old unconditional form stepped s BACK
				// by one, the trailing s++ returned it - net zero advance, infinite CPU loop that
				// froze the browser main thread forever.
				s += L - 1;
			};
		};
		s++;
		...
		if (s[0] == '%')
		{
			...
			else
			{
				...
				if (L)
				{
					...
					// same infinite-loop guard as CReadX: only skip parsed digits (L>0)
					s += L - 1;
				};
			};
		s++;
...
void ReadCoordinates( char** src, int& x, int& y, int& xR, int& yR, OneBox* BOX )
{
	...
	{
		c = ( *src )[0];
		if (!c)break;   // malformed coordinate block without ')'/']'/'}' - don't walk past the string
		if (c == 'x' && ( *src )[1] == ':')
		{
			( *src ) += 2;
			x0 = CReadX( src, x, xR );
...
extern "C" __declspec( dllexport ) char* SXP_GetAccessKey( int Index )
{
	return GetAccessKey( Index );
}

extern "C" __declspec( dllexport ) void SXP_SetAccessKeyDirect( int Index, char* Accesskey )
{
	SetAccessKey( Index, Accesskey );
}
