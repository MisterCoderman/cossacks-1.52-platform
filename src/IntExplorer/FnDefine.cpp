...
extern int mouseX;
extern int mouseY;
static inline int GetCursorPos( POINT* pt )
{
	pt->x = mouseX;
	pt->y = mouseY;
	return 1;
}
static inline DWORD GetLogicalDriveStrings( DWORD nBufferLength, char* lpBuffer )
{
...
void Draw_DBTBL(int x, int y, int Lx, int Ly, int Index, byte Active, int param)
{
	...
	for (int i = 0; i < TBL->TBL->NCol && i < nc; i++) {
		// Проверка, что индекс в пределах массива Lines. Lines хранит NLines*NCol элементов
		// (row*nc + col), поэтому граница = NLines*nc, а НЕ NLines. Прежняя проверка (>= NLines)
		// отсекала все ячейки кроме первой колонки первой строки -> текст комнат "пропадал".
		if (p0 + i >= TBL->TBL->NLines * TBL->TBL->NCol || !TBL->TBL->Lines) {
